...
● [0:00]. Introduction to Alternating Series.
     ◉ Main idea:
...
● [1:00]. 🧩 Example 1 — Definition of an alternating series. 
     ◉ Given series:
          ○ ∑[𝒏=1,∞] (-1)ⁿ⁻¹/𝒏
...
● [3:20]. General structure of an alternating series.
...
● [4:17]. Harmonic Series versus Alternating Harmonic Series.
     ◉ Harmonic Series:
          ○ ∑[𝒏=1,∞] 1/𝒏
          ○ = 1 + 1/2 + 1/3 + 1/4 + ...
          ○ diverges.
...
● [5:15]. 🧩 Example 2 — Alternating series ∑[𝒏=1,∞] (-1)ⁿ 𝒏²/(𝒏 + 1)!
     ◉ Expand the first terms:
          ○ -1/2! + 4/3! - 9/4! + 16/5! - ...
     ◉ Key observation:
          ○ the signs alternate:
...
● [7:53]. The Alternating Series Test.
...
● [9:24]. Condition 1 — Decreasing sequence.
...
● [10:30]. Condition 2 — Limit of the positive terms.
...
● [11:26]. Conclusion of the Alternating Series Test.
...
● [11:26]. Recap — How to use the Alternating Series Test.
...
● [13:58]. Graphical and conceptual justification.
...
● [19:00]. 🧩 Example 1 — Alternating Harmonic Series: ∑[𝒏=1,∞] [(-1)ⁿ⁻¹]/𝒏
...
● [21:00]. ❶ Check the limit condition.
     ◉ Compute:
          ○ limₙ→∞ 𝒂ₙ
     ◉ Since:
          ○ 𝒂ₙ = 1/𝒏
...
● [22:00]. ❷ Check the decreasing condition.
     ◉ We need:
          ○ 𝒂ₙ₊₁ ≤ 𝒂ₙ
     ◉ Since:
          ○ 𝒂ₙ = 1/𝒏
...
● [24:13]. Conclusion for Example 1.
...
● [27:52]. 🧩 Example 2 — ∑[𝒏=1,∞] (-1)ⁿ ⋅ 2𝒏/(4𝒏 - 1).
     ◉ Identify the positive part:
          ○ 𝒂ₙ = 2𝒏/(4𝒏 - 1)
     ◉ The series is alternating because of:
          ○ (-1)ⁿ

● [28:50]. Checking decreasing behavior.
...
● [29:20]. Using the derivative to show decreasing.
     ◉ Differentiate:
          ○ 𝒇(𝒙) = 2𝒙/(4𝒙 - 1)
...
● [31:42]. Check the limit condition.
     ◉ Compute:
          ○ limₙ→∞ 2𝒏/(4𝒏 - 1)
...
● [32:18]. Conclusion for Example 2.
...
● [33:31]. 🧩 Example 3 —  ∑[𝒏=2,∞] (-1)ⁿ⁻¹ ⋅ √(𝒏 + 1)/(𝒏 - 1).
     ◉ Given series: 
          ○ ∑ (-1)ⁿ⁻¹ ⋅ √(𝒏 + 1)/(𝒏 - 1)
     ◉ Identify the positive part:
          ○ 𝒂ₙ = √(𝒏 + 1)/(𝒏 - 1)
...
● [36:23]. Check the limit condition using L'Hôpital's Rule.
     ◉ Compute:
          ○ limₙ→∞ √(𝒏 + 1)/(𝒏 - 1)
     ◉ This has the form:
          ○ ∞/∞
...
● [38:45]. Check the decreasing condition using the derivative.
     ◉ Define:
          ○ 𝒇(𝒙) = √(𝒙 + 1)/(𝒙 - 1)
...
● [42:32]. Showing the derivative is negative.
     ◉ Differentiate:
          ○ 𝒇(𝒙) = √(𝒙 + 1)/(𝒙 - 1)
...
● [44:18]. Conclusion for Example 3.
     ◉ The series is alternating.
     ◉ The positive terms satisfy:
          ○ limₙ→∞ 𝒂ₙ = 0
          ○ 𝒂ₙ is decreasing.
...
● [45:28]. Finding error in alternating series.
...
● [53:36]. Alternating Series Estimation Theorem.
...
● [55:04]. Why the first omitted term controls the error.
...
● [1:01:24]. 🧩 Example 4 — Practical problem of precision and error: ∑[𝒏=0,∞] (-1)ⁿ/𝒏!
     ◉ Goal:
...
● [1:02:42]. Check convergence first.
     ◉ The series is alternating because of:
          ○ (-1)ⁿ
     ◉ Identify the positive part:
          ○ 𝒂ₙ = 1/𝒏!
...
● [1:09:50]. Finding how many terms are needed.
     ◉ We want:
          ○ |𝑹ₙ| < 0.0005
     ◉ Since:
          ○ |𝑹ₙ| ≤ 1/(𝒏 + 1)!
...
● [1:13:01]. Solving the inequality.
     ◉ Start with:
          ○ 1/(𝒏 + 1)! < 0.0005
     ◉ Take reciprocals:
          ○ (𝒏 + 1)! > 1/0.0005
...
● [1:15:56]. Computing the partial sum 𝑺₆.
